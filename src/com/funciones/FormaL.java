...
/**
 * La clase Cuadrado representa un tetramino que tiene la forma de una forma en L.
 * Extiende la clase Tetramino y se utiliza para representar dicho tetramino en el tablero del juego.
 */
public class FormaL extends Tetramino {
    //Nombre por defecto de los objetos de esta clase
    private static final String NOMBRE_POR_DEFECTO = "Forma L";

    /**
     * Crea un nuevo objeto FormaL en la posición especificada.
     *
    ...
    public FormaL(int posicionX, int posicionY) {
    ...
    public void construirNorte(int posicionX, int posicionY, Tablero tablero) {
        ...
        if (comprobarLimite(nextX - 2, nextY - 1) && tablero.comprobarCasilla(nextX - 2, nextY - 1, Estado.AGUA)) {
            contadorOK++;
        }
        ...
        if (contadorOK == PUNTOS_DE_VIDA) {
            ...
            tablero.colocarTetramino(tablero, NOMBRE_POR_DEFECTO, nextX - 2, nextY - 1, Estado.OCUPADO);
    ...
    public void construirEste(int posicionX, int posicionY, Tablero tablero) {
        ...
        if (comprobarLimite(nextX + 1, nextY + 2) && tablero.comprobarCasilla(nextX + 1, nextY + 2, Estado.AGUA)) {
            contadorOK++;
        }
        ...
        if (contadorOK == PUNTOS_DE_VIDA) {
            ...
            tablero.colocarTetramino(tablero, NOMBRE_POR_DEFECTO, nextX + 1, nextY + 1, Estado.OCUPADO);
            tablero.colocarTetramino(tablero, NOMBRE_POR_DEFECTO, nextX + 1, nextY + 2, Estado.OCUPADO);
            //Cambia su atributo a true para verificar su correcta colocacion en el tablero
            setColocado(true);
        }
        //Vuelve a poner a cero el contador para poder volver a usarlo
